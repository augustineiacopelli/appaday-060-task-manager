...
A correction to that: the first version of this actually stamped the tombstone's comparison timestamp with the deleted task's `dateCreated`, not its `lastModified`, while the check on the other side compared against `lastModified` first. Since `lastModified` is always equal to or later than `dateCreated`, that mismatch meant any task that had ever been edited, deleted afterward, would resurrect unconditionally, since its own tombstone could never be recognized as covering it. The tombstone now stamps with `lastModified || dateCreated`, the same precedence used on the comparison side, so this holds regardless of whether the deleted task had edit history.
...
### A different cause of the same symptom: browser caching of the pull
